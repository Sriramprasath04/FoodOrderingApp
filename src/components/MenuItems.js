...
import { RES_MENU_PATH } from '../utils/constants';
...
const MenuItems = () => {
    ...
    // const {name, costForTwoMessage, totalRatingsString, avgRatingString} = resData?.cards[2]?.card?.card?.info;
    const {itemCards} = resData?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[2]?.card?.card;
    console.log("=>" + resData?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[2]?.card?.card);
